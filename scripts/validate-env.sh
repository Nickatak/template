...
case "$mode" in
  ...
  docker)
    ...
    ;;
  prod)
    require_vars SECRET_KEY JWT_SECRET STAGING_DATABASE_URL STAGING_ALLOWED_HOSTS STAGING_CORS_ALLOWED_ORIGINS STAGING_NEXT_PUBLIC_API_URL STAGING_MYSQL_DATABASE STAGING_MYSQL_USER STAGING_MYSQL_PASSWORD STAGING_MYSQL_ROOT_PASSWORD
    ;;
  *)
