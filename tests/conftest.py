...
class APITestClient:
    ...
    def _make_request(self, method: str, path: str, json_data=None, **kwargs):
        ...
        request_kwargs = {k: v for k, v in kwargs.items() if k not in ['headers', 'json']}
...
def pytest_collection_modifyitems(config, items):
    ...
    # Check servers
    django_available = server_is_available(django_host.split(":")[0], int(django_host.split(":")[-1]))
    frontend_available = server_is_available(frontend_host.split(":")[0], int(frontend_host.split(":")[-1]))
    
    servers_available = django_available and frontend_available
    
    # Skip E2E tests if servers not available
    if not servers_available:
        skip_reason = "E2E servers not available (run: make run-backend && make run-frontend)"
